...
export class LotesService {
	...
	async create(createDto: CreateLoteDto): Promise<ProdutoLote> {
		...
		const loteSalvo = await this.repo.save(lote);
		...
		try {
			await this.auditoriaService.registrarInsert(
				...
				loteSalvo.id,
				loteSalvo,
				createDto.usuario_log_id || createDto.responsavel_cadastro,
			);
		} catch (error) {
			console.error('Erro ao registrar auditoria de criação de lote:', error);
		}
	...
	async update(id: number, dto: UpdateLoteDto): Promise<ProdutoLote> {
		// Busca o lote antes da atualização para auditoria
		const loteAntes = await this.repo.findOne({
			where: { id },
			relations: [
				'produto',
				'responsavelCadastro',
		...
		const loteAtualizado = await this.repo.save(lote);
		...
		try {
			await this.auditoriaService.registrarUpdate(
				...
				loteAntes,
				loteAtualizado,
				dto.usuario_log_id || loteAntes.responsavelCadastro?.id,
			);
		} catch (error) {
			console.error('Erro ao registrar auditoria de atualização de lote:', error);
		}
	...
	async remove(id: number): Promise<void> {
		...
		try {
			await this.auditoriaService.registrarDelete(
				...
				lote.id,
				lote,
				lote.usuarioLogId || lote.responsavelCadastro?.id,
			);
		} catch (error) {
			console.error('Erro ao registrar auditoria de exclusão de lote:', error);
		}
